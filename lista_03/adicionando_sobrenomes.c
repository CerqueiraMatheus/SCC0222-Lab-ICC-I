...
//Insere os nomes finais de acordo com a lógica do exercício
void swapNames(char ***names, int nameCounter, int **wordNameCounter) {
    for (int i = 0; i < nameCounter - 1; i += 2) {
        names[i + 1] = (char **)realloc(names[i + 1], ++((*wordNameCounter)[i + 1]) * sizeof(char *));
        names[i + 1][(*wordNameCounter)[i + 1] - 1] = (char *)malloc(strlen(names[i][(*wordNameCounter)[i] - 1]) + 1);
        strcpy(names[i + 1][(*wordNameCounter)[i + 1] - 1], names[i][(*wordNameCounter)[i] - 1]);
    }
}
